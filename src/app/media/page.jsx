...
const MediaPage = () => {
  ...
  const totalArticlePages = Math.ceil(
    preEventArticles.length / articlesPerPage,
  );
  ...
  return (
    <div className="pb-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <section className="mb-32">
        <h2 className="text-2xl font-bold text-white mb-12 border-l-4 border-gold pl-4 uppercase tracking-wider">
          Cinematic Archives
        </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {videos.map((video, idx) => {
            ...
            return (
              ...
              >
                <div
                  className={`${isReel ? "aspect-9/16" : "aspect-video"} w-full h-full`}
                >
                  {video.isLocal ? (
                    <video
                      src={video.url}
                ...
                <div className="absolute inset-0 bg-linear-to-t from-black/90 via-black/20 to-transparent opacity-100 group-hover:opacity-0 transition-opacity duration-500 pointer-events-none text-left">
                  <div className="absolute bottom-6 left-6 right-6">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {isReel && (
                        <span className="px-2 py-0.5 rounded-full bg-gold/20 text-gold text-[8px] font-bold uppercase tracking-widest border border-gold/30">
                          Reel
                        </span>
                      )}
                    </div>
      ...
      <section id="articles-section" className="mb-32 scroll-mt-32">
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-2xl font-bold text-white border-l-4 border-gold pl-4 uppercase tracking-wider">
            Features & Articles
          </h2>
          <span className="text-white/40 text-sm hidden sm:inline-block">
            {preEventArticles.length} Publications
          </span>
        </div>
            ...
            >
              ...
              >
                {article.image ? (
                  ...
                  <div
                    className={`w-full h-48 flex flex-col items-center justify-center shrink-0 opacity-80 group-hover:opacity-100 transition-opacity ${
                  ...
                  >
                    {article.publisher.toLowerCase().includes("instagram") ? (
                      ...
                      <span className="text-white/20 font-bold tracking-widest uppercase text-xs">
                        Article / Post
                      </span>
                    )}
                    {(article.publisher.toLowerCase().includes("instagram") ||
                ...
                <div className="p-6 flex-1 flex flex-col">
                  <div className="flex justify-between items-start mb-4 gap-2">
                    <span className="text-gold text-xs font-bold tracking-widest uppercase break-all">
                      {article.publisher}
                    </span>
      ...
      <section id="gallery-section" className="mb-32 scroll-mt-32">
        <div className="flex items-center justify-between mb-12">
          ...
          <span className="text-white/40 text-sm">
            {allGalleryImages.length} Collection
          </span>
        </div>
